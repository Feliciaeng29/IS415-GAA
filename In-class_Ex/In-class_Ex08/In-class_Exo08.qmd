...
Note:
(Always start with logitude then latitude, because we always start with the x coordinate then the y)
...
Note:
(This is the lm method based in R. SELLING_PRICE is the dependent variable then the rest will be independent variables) 
...
## In Class Exercise ggcoefstats: Visualise model parameter

Can change it to non paramedic if data is not normal distribution.
In this case we can do a sort in ascending order.
