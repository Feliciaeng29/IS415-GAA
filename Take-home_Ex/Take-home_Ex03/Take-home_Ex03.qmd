...
```{r}
pacman::p_load(tidyverse, knitr, rgdal, maptools, sf,raster,spatstat, tmap,tmaptools, gridExtra, leaflet, OpenStreetMap, ggstatsplot, statsExpressions, lubridate, raster, gridExtra, skimr, rstantools, rgeos)
```
...
```{r}
Ktest_central_hotel <- Kest(ppp_store4$central$hotel, correction = "Ripley")
Ktest_central_hotel.csr <- envelope(ppp_store4$central$hotel, Kest, nsim=99, rank=1, glocal=TRUE)

Ktest_central_entire <- Kest(ppp_store4$central$entire, correction = "Ripley")
Ktest_central_entire.csr <- envelope(ppp_store4$central$entire, Kest, nsim=99, rank=1, glocal=TRUE)

Ktest_central_private <- Kest(ppp_store4$central$private, correction = "Ripley")
Ktest_central_private.csr <- envelope(ppp_store4$central$private, Kest, nsim=99, rank=1, glocal=TRUE)

Ktest_central_shared <- Kest(ppp_store4$central$shared, correction = "Ripley")
Ktest_central_shared.csr <- envelope(ppp_store4$central$shared, Kest, nsim=99, rank=1, glocal=TRUE)
```
